<script lang="ts">
	import { useExternalMachines } from '$lib/hooks/queries/use-copy-machines.svelte.js';
	...
	const query = $derived(useExternalMachines(undefined, currentPage, pageSize));
	...
	{:else if machines.length > 0}
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Visualização de máquinas externas (não catalogadas)
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Table>
	...
	{:else}
		<Card>
			<CardContent class="py-12 text-center">
				...
				<p class="text-lg font-medium text-muted-foreground">Nenhuma máquina externa encontrada</p>
			</CardContent>
		</Card>
	{/if}
</div>
