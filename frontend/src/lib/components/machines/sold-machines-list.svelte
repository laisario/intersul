<script lang="ts">
	import { useSoldMachines } from '$lib/hooks/queries/use-copy-machines.svelte.js';
	...
	const query = $derived(useSoldMachines(undefined, currentPage, pageSize));
	...
	{:else if machines.length > 0}
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Visualização de máquinas vendidas (SOLD)
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Table>
	...
	{:else}
		<Card>
			<CardContent class="py-12 text-center">
				...
				<p class="text-lg font-medium text-muted-foreground">Nenhuma máquina vendida encontrada</p>
			</CardContent>
		</Card>
	{/if}
</div>
