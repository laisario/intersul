...
Toggles a user's active status between active and inactive. Allows soft-deletion of users without removing their data.
...
### In scope
- Core toggle user active functionality
...
1. User initiates toggle user active action
2. System validates request and permissions
3. System processes toggle user active operation
...
- Valid request successfully completes toggle user active operation
...
- `PATCH /users/:id/toggle-active`: Handles toggle user active operation
...
**Logs/events:**
- toggle user active operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- toggle user active operation frequency
