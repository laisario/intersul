...
Retrieves aggregated statistics about users such as total count, role distribution, and active/inactive counts.
...
### In scope
- Core get user stats functionality
...
1. User initiates get user stats action
2. System validates request and permissions
3. System processes get user stats operation
...
- Valid request successfully completes get user stats operation
...
- `GET /users/stats`: Handles get user stats operation
...
**Logs/events:**
- get user stats operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- get user stats operation frequency
