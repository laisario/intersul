...
Permanently deletes a user record from the system. This is a hard deletion that cannot be undone.
...
### In scope
- Core delete user functionality
...
1. User initiates delete user action
2. System validates request and permissions
3. System processes delete user operation
...
- Valid request successfully completes delete user operation
...
- `DELETE /users/:id`: Handles delete user operation
...
**Logs/events:**
- delete user operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- delete user operation frequency
