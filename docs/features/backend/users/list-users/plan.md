...
Retrieves all users in the system with optional filters (role, active status). Returns user data excluding passwords.
...
1. User initiates list users action
2. System validates request and permissions
3. System processes list users operation
...
- Valid request successfully completes list users operation
...
- `GET /users`: Handles list users operation
...
**Logs/events:**
- list users operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- list users operation frequency
