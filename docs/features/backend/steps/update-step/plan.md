...
Updates an existing step record. Allows modification of step details, notes, and status. Only the assigned responsable or admins can update.
...
### In scope
- Core update step functionality
...
1. User initiates update step action
2. System validates request and permissions
3. System processes update step operation
...
- Valid request successfully completes update step operation
...
- `PATCH /steps/:id`: Handles update step operation
...
**Logs/events:**
- update step operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- update step operation frequency
