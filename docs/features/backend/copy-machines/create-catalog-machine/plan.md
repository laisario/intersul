# Feature: Create Catalog Machine

## Feature summary

Creates a new copy machine in the catalog with manufacturer, model, features, and optional image. Supports image upload during creation.
...
### In scope
- Core create catalog machine functionality
...
1. User initiates create catalog machine action
2. System validates request and permissions
3. System processes create catalog machine operation
...
- Valid request successfully completes create catalog machine operation
...
- `POST /copy-machines/catalog`: Handles create catalog machine operation
...
**Logs/events:**
- create catalog machine operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- create catalog machine operation frequency
