# Feature: Delete Catalog Machine

## Feature summary

Permanently deletes a catalog machine record. May fail if machine is in use by client machines.
...
### In scope
- Core delete catalog machine functionality
...
1. User initiates delete catalog machine action
2. System validates request and permissions
3. System processes delete catalog machine operation
...
- Valid request successfully completes delete catalog machine operation
...
- `DELETE /copy-machines/catalog/:id`: Handles delete catalog machine operation
...
**Logs/events:**
- delete catalog machine operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- delete catalog machine operation frequency
