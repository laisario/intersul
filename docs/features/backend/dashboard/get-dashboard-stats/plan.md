...
Retrieves dashboard statistics including service counts, step statuses, client metrics, and other aggregated data. Admin only. Supports force recalculation.
...
### In scope
- Core get dashboard stats functionality
...
1. User initiates get dashboard stats action
2. System validates request and permissions
3. System processes get dashboard stats operation
...
- Valid request successfully completes get dashboard stats operation
...
- `GET /dashboard/stats`: Handles get dashboard stats operation
...
**Logs/events:**
- get dashboard stats operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- get dashboard stats operation frequency
