...
Retrieves a specific category by ID including name and step templates.
...
### In scope
- Core get category functionality
...
1. User initiates get category action
2. System validates request and permissions
3. System processes get category operation
...
- Valid request successfully completes get category operation
...
- `GET /categories/:id`: Handles get category operation
...
**Logs/events:**
- get category operations should be logged
- Failed attempts can be logged
- Error conditions should be logged

**Metrics (optional):**
- get category operation frequency
