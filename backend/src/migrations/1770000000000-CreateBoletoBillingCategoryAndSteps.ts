...
export class CreateBoletoBillingCategoryAndSteps1770000000000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (category && category.length > 0) {
      ...
      if (
        (hasSteps[0]?.count || 0) === 0 &&
        (hasServices[0]?.count || 0) === 0
      ) {
        await queryRunner.query(`DELETE FROM categories WHERE id = ?`, [
          categoryId,
        ]);
      }
    }
